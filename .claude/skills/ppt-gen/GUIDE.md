...
| 파일 | 역할 | Claude 참조 시점 |
|------|------|-----------------|
| `SKILL.md` | 엔트리포인트, 워크플로우 선택 (~80줄) | 모든 PPT 요청 시 |
| `workflows/*.md` | 워크플로우별 상세 가이드 (11개) | 해당 작업 시 |
| `html2pptx.md` | HTML → PPTX 변환 상세 규칙 | 새 PPT 생성 시 |
| `ooxml.md` | XML 직접 편집 상세 규칙 | 기존 PPT 수정 시 |

#### workflows/ 디렉토리 (NEW)
| 파일 | 용도 |
|------|------|
| `analysis.md` | PPT 분석/읽기 |
| `html2pptx.md` | 새 PPT 생성 (템플릿 없음) |
| `ooxml.md` | 기존 PPT 편집 |
| `template.md` | 템플릿 기반 생성 |
| `content-extract.md` | 콘텐츠 템플릿 추출 (v2.0) |
| `document-extract.md` | 문서 템플릿 추출 |
| `style-extract.md` | 이미지 스타일 추출 |
| `design-search.md` | 디자인 레퍼런스 검색 |
...
| `template-analyzer.py` | PPTX → YAML | document-extract |
...
#### WF4: content-extract (콘텐츠 추출)

**트리거:** "콘텐츠 추출해줘", "슬라이드 저장해줘", "레이아웃 추출해줘"
...
2. 디자인 분석
   └→ design_intent 분류 (40개 카테고리)
   └→ quality_score 평가 (0-10)
   └→ 공간 관계 분석
...
4. YAML 저장 (v2.0 스키마)
   └→ templates/contents/templates/{id}.yaml
   └→ templates/contents/thumbnails/{id}.png
   └→ registry.yaml 업데이트
```

**출력 스키마:** `references/content-schema.md` 참조

---

#### WF5: document-extract (문서 양식 추출)

**트리거:** "문서 양식 추출해줘", "템플릿 등록해줘"
...
**역할:** PPTX 슬라이드 썸네일 생성 (검증용 및 콘텐츠 추출용)

**사용법:**
```bash
python scripts/thumbnail.py <input.pptx> <output_dir> [options]
...
**옵션:**
- `--cols N`: 그리드 컬럼 수 (기본: 5, 최대: 6)
- `--slides N`: 추출할 슬라이드 인덱스 (0-based, 쉼표로 구분)
- `--single`: 단일 슬라이드 고해상도 출력 (1980x1080 PNG)
- `--outline-placeholders`: 텍스트 영역에 빨간 테두리 표시

**출력:**
- 그리드 모드: `{prefix}.jpg` 또는 `{prefix}-N.jpg` (다중 그리드)
- 단일 모드: `{output_dir}/slide-{N}.png` (1980x1080)
...
| content-schema.md | `references/` | 콘텐츠 템플릿 v2.0 스키마 |
