...
## 14. Output 폴더 및 파이프라인 추적 시스템

### 13.1 개요

PPT 생성 시 **단계별 JSON 파일**에 중간 과정을 저장하고 실시간으로 업데이트합니다.

**핵심 원칙**:
- 각 단계별 독립 JSON 파일로 분리
- 세션 재개 가능 (중단된 작업 이어서 진행)
- 단계별 디버깅 및 검증 용이
- 특정 단계만 재실행 가능

### 13.2 Output 폴더 구조 (단계별 파일 분리)
...
└─────────────────────────────────────────────────────────────────────────┘
    ...
    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ 4단계: 이미지 생성 (Image Generation) - 선택적                            │
│ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                                   │
│ • 입력: 3단계 출력 JSON (이미지 생성 프롬프트 포함)                         │
│ • 출력: 이미지 생성 서비스로 생성된 이미지                                 │
│                                                                         │
│ ⚠️ 현재 미구현 - Skip 가능                                                │
│ • 이미지가 필요한 슬라이드는 텍스트 박스로 대체                            │
│                                                                         │
│ 📄 저장: stage-4-images.json (생성된 이미지 경로)                         │
└─────────────────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
### 13.4 단계별 JSON 파일 스키마

#### session.json (세션 메타)
```json
{
  "id": "2026-01-08_143025_a7b2c3d4",
  "created_at": "2026-01-08T14:30:25Z",
...
```json
{
  ...
  "name": "setup",
  "status": "completed",
  "completed_at": "2026-01-08T14:30:30Z",
  "presentation": {
...
```json
{
  ...
  "name": "outline",
  "status": "completed",
  "completed_at": "2026-01-08T14:31:00Z",
  "slides": [
    {
      "index": 0,
      "title": "스마트 물류 시스템",
      "purpose": "cover",
      "key_points": ["제안사", "날짜", "버전"],
      "speaker_notes": ""
    }
  ]
}
...
```json
{
  ...
  "status": "completed",
  "completed_at": "2026-01-08T14:31:30Z",
  "matches": [
    {
      "slide_index": 0,
      "content_type": "cover",
...
2. 사용자가 재개할 세션 선택
3. `session.json`에서 `current_stage` 확인
4. 해당 단계의 stage-N-*.json 파일부터 이어서 진행
...
- [ ] **1.4 테스트**
  - [ ] content-extract 워크플로우 E2E 테스트
  - [ ] document-extract 워크플로우 테스트
  - [ ] style-extract 워크플로우 테스트
...
- [ ] **2.2 스크립트 정리**
  - [ ] 추출 관련 스크립트 제거 (ppt-extract로 이동됨)
  - [ ] 생성 관련 스크립트만 유지

- [ ] **2.3 테스트**
  - [ ] html2pptx 파이프라인 E2E 테스트
  - [ ] 템플릿 기반 생성 테스트
